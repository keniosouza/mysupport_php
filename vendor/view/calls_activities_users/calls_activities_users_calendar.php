...
$userId = @(int)filter_input(INPUT_POST, 'USER_ID', FILTER_SANITIZE_SPECIAL_CHARS);

/** Busco todos os registros */
$CallsActivitiesUsersAllCalendarByUserResult = $CallsActivitiesUsers->AllCalendarByUser($userId);

/** Variavel para guardar os dados do calendário */
$resultCalendar = [];

/** Percorro todos os registros */
foreach ($CallsActivitiesUsersAllCalendarByUserResult as $key => $result) {

    /** Formato os dados para estrutura do calendário */
    $resultTemp = new stdClass();
    $resultTemp->title = '#' . $result->call_activity_id . '-'. $result->activity_name;
    $resultTemp->start = !empty($result->call_activity_date_close) ? $result->call_activity_date_close : $result->call_activity_date_expected;
    $resultTemp->groupId = (int)$result->call_activity_user_id;
    $resultTemp->className = empty($result->call_activity_date_close) ? 'bg-primary' : 'bg-success';
    $resultTemp->extendedProps = new stdClass();
    $resultTemp->extendedProps->call_activity_user_id = $result->call_activity_user_id;
