...
if (!empty($CallsActivitiesValidate->getErrors())) {
    ...
    $SectionsGetResult = $Sections->Get($CallsActivitiesValidate->getSectionId());

    /** Verifico se o registro foi localizado */
    if ($CallsActivities->SaveSectionId($CallsActivitiesValidate->getCallActivityId(), $CallsActivitiesValidate->getSectionId())) {
        ...
        if (count($CallsActivitiesUsersAllByActivityIdResult) > 0)
        {
            ...
            foreach ($CallsActivitiesUsersAllByActivityIdResult as $key => $result)
            {
                ...
                $NotificationsValidate->setData(json_encode(['TITLE' => 'Alterado!', 'MESSAGE' => 'Seção da Atividade "'. '#' . $result->call_activity_id . ' - ' . $result->activity_name . '" Alterada para : ' . $SectionsGetResult->name . '!', 'CLASS' => 'success', 'ICON' => 'image/default/checked.svg'], JSON_PRETTY_PRINT));
        ...
        $MessagesValidate->setData('Alterando seção para: ' . $SectionsGetResult->name);
        ...
        $result = [
            ...
            'procedure' => [
                [
                    'name' => 'PutHtml',
                    'options' => [
                        'target' => 'CallActivitySection' . $CallsActivitiesValidate->getCallActivityId(),
                        'data' => $SectionsGetResult->name
