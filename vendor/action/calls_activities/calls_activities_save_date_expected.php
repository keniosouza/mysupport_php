...
$CallsActivitiesValidate->setDateExpected(@(string)filter_input(INPUT_POST, 'date_expected', FILTER_SANITIZE_NUMBER_INT));
...
if (!empty($CallsActivitiesValidate->getErrors())) {
    ...
    if ($CallsActivities->SaveDateExpected($CallsActivitiesValidate->getCallActivityId(), $CallsActivitiesValidate->getDateExpected())) {
        ...
        if (count($CallsActivitiesUsersAllByActivityIdResult) > 0)
        {
            ...
            foreach ($CallsActivitiesUsersAllByActivityIdResult as $key => $result)
            {
                ...
                $NotificationsValidate->setData(json_encode(['TITLE' => 'Alterado!', 'MESSAGE' => 'Data de Conclusão da Atividade "'. '#' . $result->call_activity_id . ' - ' . $result->activity_name . '" Alterada para : ' . date('d/m/Y', strtotime( $CallsActivitiesValidate->getDateExpected())) . '!', 'CLASS' => 'success', 'ICON' => 'image/default/checked.svg'], JSON_PRETTY_PRINT));
        ...
        $MessagesValidate->setData('Alterando data de conclusão para: ' . date('d/m/Y', strtotime( $CallsActivitiesValidate->getDateExpected())));
        ...
        $result = [
            ...
            'procedure' => [
                [
                    'name' => 'PutHtml',
                    'options' => [
                        'target' => 'CallActivityDateExpected' . $CallsActivitiesValidate->getCallActivityId(),
                        'data' => date('d/m/Y', strtotime( $CallsActivitiesValidate->getDateExpected()))
                    ]
                ]
            ],
