...
$CallsActivitiesValidate->setCallActivityLevelId(@(int)filter_input(INPUT_POST, 'call_activity_level_id', FILTER_SANITIZE_NUMBER_INT));
...
if (!empty($CallsActivitiesValidate->getErrors())) {
    ...
    $CallsActivitiesLevelsGetResult = $CallsActivitiesLevels->get($CallsActivitiesValidate->getCallActivityLevelId());

    /** Verifico se o registro foi localizado */
    if ($CallsActivities->SaveCallActivityLevelId($CallsActivitiesValidate->getCallActivityId(), $CallsActivitiesValidate->getCallActivityLevelId())) {
        ...
        if (count($CallsActivitiesUsersAllByActivityIdResult) > 0)
        {
            ...
            foreach ($CallsActivitiesUsersAllByActivityIdResult as $key => $result)
            {
                ...
                $NotificationsValidate->setData(json_encode(['TITLE' => 'Alterado!', 'MESSAGE' => 'Nível da Atividade "'. '#' . $result->call_activity_id . ' - ' . $result->activity_name . '" Alterada para : ' . $CallsActivitiesLevelsGetResult->description . '!', 'CLASS' => 'success', 'ICON' => 'image/default/checked.svg'], JSON_PRETTY_PRINT));
        ...
        $MessagesValidate->setData('Alterando nível para: ' . $CallsActivitiesLevelsGetResult->description);
        ...
        $result = [
            ...
            'procedure' => [
                [
                    'name' => 'PutHtml',
                    'options' => [
                        'target' => 'CallActivityLevel' . $CallsActivitiesValidate->getCallActivityId(),
                        'data' => $CallsActivitiesLevelsGetResult->description
                    ]
                ]
            ]
        ];
