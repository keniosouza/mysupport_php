...
$CallsActivitiesValidate->setCallActivityTypeId(@(int)filter_input(INPUT_POST, 'call_activity_type_id', FILTER_SANITIZE_NUMBER_INT));
...
if (!empty($CallsActivitiesValidate->getErrors())) {
    ...
    $CallsActivitiesTypesGetResult = $CallsActivitiesTypes->Get($CallsActivitiesValidate->getCallActivityTypeId());

    /** Verifico se o registro foi localizado */
    if ($CallsActivities->SaveCallActivityTypeId($CallsActivitiesValidate->getCallActivityId(), $CallsActivitiesValidate->getCallActivityTypeId())) {
        ...
        if (count($CallsActivitiesUsersAllByActivityIdResult) > 0)
        {
            ...
            foreach ($CallsActivitiesUsersAllByActivityIdResult as $key => $result)
            {
                ...
                $NotificationsValidate->setData(json_encode(['TITLE' => 'Alterado!', 'MESSAGE' => 'Categoria da Atividade "'. '#' . $result->call_activity_id . ' - ' . $result->activity_name . '" Alterada para : ' . $CallsActivitiesTypesGetResult->description . '!', 'CLASS' => 'success', 'ICON' => 'image/default/checked.svg'], JSON_PRETTY_PRINT));
        ...
        $MessagesValidate->setData('Alterando categoria para: ' . $CallsActivitiesTypesGetResult->description);
        ...
        /** Log de requisições */
        $Logs->Save($LogsValidate->getLogId(), 4, $LogsValidate->getCompanyId(), $LogsValidate->getUserId(), 'calls_activities', $CallsActivitiesValidate->getCallActivityId(), md5('calls' . $CallsActivitiesGetResult->call_id), $LogsHandling->getDefaultData('Comentário!', ' Novo comentário publicado!', 'warning'), $LogsValidate->getDateRegister());
        ...
        $result = [
            ...
            'procedure' => [
               [
                   'name' => 'PutHtml',
                   'options' => [
                       'target' => 'CallActivityType' . $CallsActivitiesValidate->getCallActivityId(),
                       'data' => $CallsActivitiesTypesGetResult->description
                   ]
               ]
            ]
