...
$CallsActivitiesValidate->setCallActivityPriorityId(@(int)filter_input(INPUT_POST, 'call_activity_priority_id', FILTER_SANITIZE_NUMBER_INT));
...
if (!empty($CallsActivitiesValidate->getErrors())) {
    ...
    $CallsActivitiesPrioritiesGetResult = $CallsActivitiesPriorities->Get($CallsActivitiesValidate->getCallActivityPriorityId());

    /** Verifico se o registro foi localizado */
    if ($CallsActivities->SaveCallActivityPriorityId($CallsActivitiesValidate->getCallActivityId(), $CallsActivitiesValidate->getCallActivityPriorityId())) {
        ...
        if (count($CallsActivitiesUsersAllByActivityIdResult) > 0)
        {
            ...
            foreach ($CallsActivitiesUsersAllByActivityIdResult as $key => $result)
            {
                ...
                $NotificationsValidate->setData(json_encode(['TITLE' => 'Alterado!', 'MESSAGE' => 'Prioridade da Atividade "'. '#' . $result->call_activity_id . ' - ' . $result->activity_name . '" Alterada para : ' . $CallsActivitiesPrioritiesGetResult->description . '!', 'CLASS' => 'success', 'ICON' => 'image/default/checked.svg'], JSON_PRETTY_PRINT));
        ...
        $MessagesValidate->setData('Alterando prioriade para: ' . $CallsActivitiesPrioritiesGetResult->description);
            ...
            'procedure' => [
                [
                    ...
                    'options' => [
                        ...
                        'target' => 'CallActivityPriority' . $CallsActivitiesValidate->getCallActivityId(),
                        'data' => $CallsActivitiesPrioritiesGetResult->description
