...
$CallsValidate->setDateClose(date('Y-m-d'));
        ...
        if ($Calls->SaveClose($CallsValidate->getCallId(), $CallsValidate->getDateClose())) {
            ...
            if (count($CallsActivitiesUsersAllUserIdByCallIdResult) > 0)
            {
                ...
                foreach ($CallsActivitiesUsersAllUserIdByCallIdResult as $key => $result)
                {
                    ...
                    $NotificationsValidate->setData(json_encode(['TITLE' => 'Encerrado!', 'MESSAGE' => 'Chamado "'. '#' . $result->call_id . ' - ' . $result->name . '" Encerrado!', 'CLASS' => 'success', 'ICON' => 'image/default/checked.svg'], JSON_PRETTY_PRINT));
            ...
            $Logs->Save($LogsValidate->getLogId(), 4, $LogsValidate->getCompanyId(), $LogsValidate->getUserId(), 'calls', $CallsGetResult->call_Id, md5('calls' . $CallsGetResult->call_id), $LogsHandling->getDefaultData('Encerrado!', 'Chamado <b>"' . $CallsGetResult->name . '"</b> Encerrada!', 'success'), $LogsValidate->getDateRegister());
