<?php

/** Importação de classes */
use vendor\model\UsersAcls;
use vendor\controller\users_acls\UsersAclsValidate;

/** Instânciamento de classes */
$UsersAcls = new UsersAcls();
$UsersAclsValidate = new UsersAclsValidate();

/** Percorro todos os registros */
foreach ($_POST['modules_acls_id'] as $keyResult => $result) {

    /** Parâmetros de entrada */
    $UsersAclsValidate->setUsersAclsId(@(int)filter_input(INPUT_POST, 'users_acls_id', FILTER_SANITIZE_SPECIAL_CHARS));
    $UsersAclsValidate->setUsersId(@(int)filter_input(INPUT_POST, 'users_id', FILTER_SANITIZE_SPECIAL_CHARS));
    $UsersAclsValidate->setModulesAclsId($result);

    /** Verifico a existência de erros */
    if (!empty($UsersAclsValidate->getErrors())) {

        /** Retorno mensagem de erro */
        throw new InvalidArgumentException($UsersAclsValidate->getErrors(), 0);

    } else {

        /** Verifico se o usuário foi localizado */
        if ($UsersAcls->Save($UsersAclsValidate->getUsersAclsId(), $UsersAclsValidate->getModulesAclsId(), $UsersAclsValidate->getUsersId())) {

            /** Result **/
            $result = [

                'code' => 200,
                'title' => 'Sucesso',
                'data' => 'Permissões vinculadas com sucesso',
                'target' => 'UsersAclsFormMessages',
                'toast' => true

            ];

        } else {

            /** Retorno mensagem de erro */
            throw new InvalidArgumentException('Não foi possivel salvar o registro', 0);

        }

    }

}

/** Envio **/
echo json_encode($result);

/** Paro o procedimento **/
exit;