...
try {
    ...
    if (!empty($CallsActivitiesUsersValidate->getErrors())) {
        ...
        if ($Files->Delete((int)$resultFiles->file_id)) {
            ...
            /** Log de requisições */
            $Logs->Save($LogsValidate->getLogId(), 4, $LogsValidate->getCompanyId(), $LogsValidate->getUserId(), 'calls_activities_users_files', $CallsActivitiesUsersValidate->getCallActivityUserId(), md5('calls' . $resultCallsActivitiesUsers->call_id), $LogsHandling->getDefaultData('Removido!', 'Arquivo removido!', 'danger'), $LogsValidate->getDateRegister());
